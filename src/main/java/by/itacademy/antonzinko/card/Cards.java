...
public class Cards {
    ...
    public static boolean isValid(Card card) {
        if(card.getExpiredData().matches("[0-1][0-9]+/[0-9][0-9]")) {
            int currentMonth = Calendar.getInstance().get(Calendar.MONTH) + 1;
            int currentYear = Calendar.getInstance().get(Calendar.YEAR) % 100;
            int cardMonth = Integer.parseInt(card.getExpiredData().split("/")[0]);
            int cardYear = Integer.parseInt(card.getExpiredData().split("/")[1]);
